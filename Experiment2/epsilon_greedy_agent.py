"""
Epsilon-Greedy Bandit Agent for Congestion Game Simulation
==========================================================
...
uses an epsilon-greedy strategy to balance exploration (random path) with
exploitation (lowest estimated latency).

Key components:
  1. EpsilonGreedyAgent      – bandit agent with sample-average updates
...
# ─────────────────────────────────────────────────────────────────────
# 1. Epsilon-Greedy Bandit Agent
# ─────────────────────────────────────────────────────────────────────

class EpsilonGreedyAgent:
    """
    An epsilon-greedy multi-armed bandit agent for path selection in a
    congestion game.

    Each available path is treated as an arm.  The agent maintains running
    sample-average estimates of latency for every path and, at each step,
    explores (picks a random path) with probability *epsilon* or exploits
    (picks the path with the lowest estimated latency) with probability
    1 − epsilon.
    ...
    def __init__(self, agent_id, paths, epsilon=0.1):
        if not paths:
    ...
    def choose_path(self):
        """
        Select a path using the epsilon-greedy policy.

        With probability *epsilon* the agent explores (uniform random);
        otherwise it exploits by choosing the path with the lowest
        estimated latency.  Ties in exploitation are broken randomly.
        ...
        if random.random() < self.epsilon:
            # Explore: pick a uniformly random path
            self.current_path_idx = random.randint(0, self.n_paths - 1)
        else:
            # Exploit: pick path with the lowest estimated latency
            min_est = np.min(self.estimates)
            best_indices = np.where(self.estimates == min_est)[0]
...
def run_mixed_simulation(G, source, sink, n_agents=50, pct_bandit=0.5,
                         epsilon=0.1, n_steps=300, opt_cost=None):
    """
    Run a mixed simulation where a fraction of agents use epsilon-greedy
    bandit learning and the rest are selfish (greedy best-response).
    ...
    pct_bandit : float, optional
        Fraction of agents that are epsilon-greedy bandits (default 0.5).
    epsilon : float, optional
        Exploration probability for bandit agents (default 0.1).
    ...
    bandit_agents = [
        EpsilonGreedyAgent(agent_id=i, paths=all_paths, epsilon=epsilon)
...
def run_poa_vs_fraction_experiment(G, source, sink, n_agents=50,
                                   fractions=None, epsilon=0.1,
                                   n_steps=300, n_repeats=10,
                                   opt_cost=None):
    """
    Sweep the fraction of epsilon-greedy agents from 0% to 100% and
...
def plot_poa_vs_fraction(results_dict, save_path='poa_vs_rl_fraction.png'):
    ...
    epsilon-greedy agents, with ±1 std shading.
    ...
    ax.set_xlabel('Fraction of Epsilon-Greedy Agents (%)')
    ax.set_ylabel('Mean Empirical PoA')
    ax.set_title('PoA vs Fraction of Epsilon-Greedy Agents')
    ax.legend(loc='best')
    ax.set_xlim(-2, 102)
...
def plot_poa_convergence(poa_history_dict, save_path='poa_convergence.png'):
    ...
    ax.set_title('PoA Convergence Over Time')
...
if __name__ == '__main__':
    ...
    print("\n" + "=" * 60)
    print("Mixed Simulation  (50% bandit, 50% selfish)")
    print("=" * 60)
    ...
    print("\n" + "=" * 60)
    print("PoA vs Fraction of Epsilon-Greedy Agents")
    print("=" * 60)
