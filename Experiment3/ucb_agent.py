"""
UCB (Upper Confidence Bound) Bandit Agent for Congestion Game Simulation
========================================================================
...
uses the UCB1 strategy to balance exploration with exploitation.

For a *minimisation* problem the agent picks the arm with the smallest
*lower confidence bound*:

    LCB_i = Q_i  −  c · √( ln(t) / N_i )

where
  - Q_i  = sample-average latency estimate for path i
  - N_i  = number of times path i has been selected
  - t    = total number of selections so far (across all arms)
  - c    = exploration parameter (controls width of confidence interval)

Arms that have never been tried are selected first (round-robin
initialisation), ensuring every path is explored at least once before
the UCB formula is applied.

Key components:
  1. UCBAgent                 – bandit agent with UCB1 selection
...
class UCBAgent:
    """
    A UCB1 multi-armed bandit agent for path selection in a congestion
    game.

    Each available path is treated as an arm.  The agent maintains running
    sample-average estimates of latency for every path.  It selects the
    path with the lowest *lower confidence bound* (LCB):

        LCB_i = Q_i  −  c · sqrt( ln(t) / N_i )

    Unvisited arms are selected first in round-robin order so that every
    path is tried at least once before the UCB formula kicks in.
    ...
    c : float, optional
        ...
        interval (default 1.41 ≈ √2, the classic UCB1 constant).
    ...
    estimates : numpy.ndarray
        Sample-average estimated latency for each path.
    current_path_idx : int
        Index of the most recently chosen path.
    total_pulls : int
        Total number of arm selections so far (across all arms).
    """

    def __init__(self, agent_id, paths, c=1.41):
        if not paths:
        ...
        self.estimates = np.zeros(self.n_paths, dtype=np.float64)
        self.total_pulls = 0
    ...
    def choose_path(self):
        """
        Select a path using the UCB1 policy (adapted for minimisation).

        Phase 1 — Initialisation:
            If any path has never been tried, pick the first such path
            (round-robin).  This guarantees every arm is explored at
            least once.

        Phase 2 — UCB selection:
            Compute  LCB_i = Q_i  −  c · sqrt(ln(t) / N_i)  for each
            arm and select the arm with the *smallest* LCB.
        ...
        # Phase 1: initialise — try every arm at least once
        untried = np.where(self.counts == 0)[0]
        if len(untried) > 0:
            self.current_path_idx = int(untried[0])
            return self.paths[self.current_path_idx]

        # Phase 2: UCB selection (minimisation → lowest LCB wins)
        log_total = math.log(self.total_pulls)
        lcb_values = (
            self.estimates - self.c * np.sqrt(log_total / self.counts)
        )
        min_lcb = np.min(lcb_values)
        best_indices = np.where(lcb_values == min_lcb)[0]
    ...
    def update(self, latency_experienced):
        """
        Update the sample-average estimate for the most recently chosen
        path using an incremental running-mean formula.
        ...
        self.counts[idx] += 1
        self.total_pulls += 1
        # Incremental sample average: Q_{n+1} = Q_n + (r - Q_n) / n
        self.estimates[idx] += (
            (latency_experienced - self.estimates[idx]) / self.counts[idx]
        )
...
def run_mixed_simulation(G, source, sink, n_agents=50, pct_bandit=0.5,
                         c=1.41, n_steps=300, opt_cost=None):
    """
    Run a mixed simulation where a fraction of agents use UCB bandit
    learning and the rest are selfish (greedy best-response).
    ...
    internal estimates and update after observing realised latency.
    ...
    pct_bandit : float, optional
        Fraction of agents that are UCB bandits (default 0.5).
    c : float, optional
        Exploration parameter for UCB agents (default 1.41 ≈ √2).
    ...
    bandit_agents = [
        UCBAgent(agent_id=i, paths=all_paths, c=c)
...
def run_poa_vs_fraction_experiment(G, source, sink, n_agents=50,
                                   fractions=None, c=1.41,
                                   n_steps=300, n_repeats=10,
                                   opt_cost=None):
    """
    Sweep the fraction of UCB agents from 0% to 100% and record the
    resulting mean final PoA.
    ...
    opt_cost : float or None, optional
        Pre-computed social-optimum cost.  If None, falls back to
        the heuristic ``_estimate_opt_cost``.
    ...
    # Use caller-supplied opt_cost, or fall back to heuristic
    if opt_cost is None:
        opt_cost = _estimate_opt_cost(G, source, sink, n_agents)
...
def _estimate_opt_cost(G, source, sink, n_agents):
    """
    Estimate the social-optimum cost by sampling random assignments
    and single-path assignments, taking the minimum observed total
    cost.  This is a rough heuristic -- for exact results the caller
    should supply an analytical opt_cost.
    ...
    for _ in range(20):
        # Random assignment
        chosen = [random.choice(all_paths) for _ in range(n_agents)]
        flow = compute_flow_dict(chosen)
        cost = sum(compute_path_latency(G, p, flow) for p in chosen)
        best_cost = min(best_cost, cost)

    # Also try all agents on each single path
    for path in all_paths:
        chosen = [path] * n_agents
        flow = compute_flow_dict(chosen)
        cost = sum(compute_path_latency(G, p, flow) for p in chosen)
        best_cost = min(best_cost, cost)

    return max(best_cost, 1e-10)
...
def plot_poa_vs_fraction(results_dict, save_path='poa_vs_rl_fraction.png'):
    ...
    ax.set_xlabel('Fraction of UCB Agents (%)')
    ax.set_ylabel('Mean Empirical PoA')
    ax.set_title('PoA vs Fraction of UCB Agents')
    ax.legend(loc='best')
    ax.set_xlim(-2, 102)
...
def plot_poa_convergence(poa_history_dict, save_path='poa_convergence.png'):
    ...
    ax.set_title('PoA Convergence Over Time (UCB Agents)')
